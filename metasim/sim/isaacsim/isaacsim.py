...
class IsaacsimHandler(BaseSimHandler):
    ...
    def _apply_mdl_material(self, obj: Any, material_config: dict) -> bool:
        ...
        try:
            ...
            # Create unique material path (consistent with PBR materials)
            ...
            if material_prim.IsValid():
                ...
                # Create MDL material using Isaac Lab commands
                try:
                    success, result = omni.kit.commands.execute(
                        "CreateMdlMaterialPrim",
                        mtl_url=full_mdl_path,
                        mtl_name=unique_material_name,
                        mtl_path=material_path,
                        select_new_prim=False,
                    )
                    if not success:
                        log.warning(f"Failed to create MDL material at {material_path}")
                        return False
                    log.debug(f"Successfully created MDL material at {material_path}")

                except Exception as e:
                    log.warning(f"Failed to create MDL material at {material_path}: {e}")
                    return False
            ...
            if success_count > 0:
                log.debug(
                    f"✅ Applied MDL material '{unique_material_name}' to {getattr(obj, 'name', 'unnamed')} (bindings: {success_count}/{self.num_envs})"
                )
                return True
            else:
                log.debug(f"❌ MDL material binding failed for {getattr(obj, 'name', 'unnamed')}")
                return False
    ...
    def _apply_pbr_material(self, obj: Any, material_config: dict) -> bool:
        ...
        try:
            ...
            if success_count > 0:
                log.debug(
                    f"✅ Applied PBR material '{unique_material_name}' to {getattr(obj, 'name', 'unnamed')} (bindings: {success_count}/{self.num_envs})"
                )
                return True
            else:
    ...
    def _apply_physics_material_to_object(self, obj: Any, physics_config: dict) -> bool:
        ...
        try:
            ...
            if success_count > 0:
                log.debug(
                    f"✅ Applied physics material '{material_name}' to {getattr(obj, 'name', 'unnamed')} (bindings: {success_count}/{self.num_envs})"
                )
                return True
            else:
            ...
            if stage:
                ...
                for obj in self.objects:
                    ...
                    if obj_name in self.scene.rigid_objects:
                        ...
                        for env_idx in range(min(3, self.num_envs)):  # Check first 3 envs
    ...
    def _verify_material_binding(self, obj_name: str, expected_material_path: str) -> bool:
    ...
    def _verify_physics_material_binding(self, obj_name: str, expected_material_path: str) -> bool:
