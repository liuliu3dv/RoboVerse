...
class KettleCfg(BaseRLTaskCfg):
    ...
    def observation_fn(self, envstates: list[EnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        t += self.action_shape // 2
        if self.use_prio:
            obs[:, t : t + 13] = envstates.objects["kettle"].root_state
            obs[:, t + 10 : t + 13] *= self.vel_obs_scale  # object1 angvel
            t += 13
            obs[:, t : t + 13] = envstates.objects["bucket"].root_state
            obs[:, t + 10 : t + 13] *= self.vel_obs_scale  # object2 angvel
            t += 13
