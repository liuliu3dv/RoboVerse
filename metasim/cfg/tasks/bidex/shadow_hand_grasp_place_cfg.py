...
@configclass
class ShadowHandGraspPlaceCfg(BaseRLTaskCfg):
    """class for bidex shadow grasp and place tasks."""
    ...
    handle_name = "link_1"
    handle_idx = None  # Index of the handle in the body state
    ...
    def observation_fn(self, envstates: list[EnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        Returns:
            ...
            424 - 426   bucket handle position
            427 - 430   bucket handle rotation
            431 - :     visual observation, currently RGB image (3 x 256 x 256)
        """
        if device is None:
            device = self.device
        ...
        obs[:, 398:411] = envstates.objects["bucket"].root_state
        obs[:, 408:411] *= self.vel_obs_scale  # object1 angvel
        obs[:, 411:424] = envstates.objects[self.current_object_type].root_state
        obs[:, 421:424] *= self.vel_obs_scale  # object2 angvel
        if self.handle_idx is None:
            self.handle_idx = envstates.objects["bucket"].body_names.index(self.handle_name)
        handle_pos = envstates.objects["bucket"].body_state[:, self.handle_idx, :3]
        handle_rot = envstates.objects["bucket"].body_state[:, self.handle_idx, 3:7]
        obs[:, 424:427] = handle_pos
        obs[:, 427:431] = handle_rot
    ...
    def reward_fn(
        ...
        Returns:
            ...
            success_buf (torch.Tensor): The success buffer of all environments at this time, shape (num_envs,)
        """
        bucket_pos = envstates.objects["bucket"].body_state[:, self.handle_idx, :3]
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (
        torch.norm(right_object_pos - right_hand_ff_pos, p=2, dim=-1)
        + torch.norm(right_object_pos - right_hand_mf_pos, p=2, dim=-1)
        + torch.norm(right_object_pos - right_hand_rf_pos, p=2, dim=-1)
        + torch.norm(right_object_pos - right_hand_lf_pos, p=2, dim=-1)
        + torch.norm(right_object_pos - right_hand_th_pos, p=2, dim=-1)
    )
    left_hand_finger_dist = (
        torch.norm(left_object_pos - left_hand_ff_pos, p=2, dim=-1)
        + torch.norm(left_object_pos - left_hand_mf_pos, p=2, dim=-1)
        + torch.norm(left_object_pos - left_hand_rf_pos, p=2, dim=-1)
        + torch.norm(left_object_pos - left_hand_lf_pos, p=2, dim=-1)
    ...
    up_rew = torch.exp(-10 * torch.norm(right_object_pos - left_object_pos, p=2, dim=-1)) * 2

    reward = right_hand_dist_rew + left_hand_dist_rew + up_rew

    success = torch.norm(right_object_pos - left_object_pos, p=2, dim=-1) < 0.2
