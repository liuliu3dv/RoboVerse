...
class ShadowHandCatchUnderarmCfg(BaseRLTaskCfg):
    ...
    robots = [
        ShadowHandCfg(
            ...
            actuated_link="robot0_hand mount",
        ),
        ShadowHandCfg(
    ...
    def set_objects(self) -> None:
        """Set the objects for the shadow hand catch underarm task."""
        self.objects.append(self.objects_cfg[self.current_object_type])

    def set_init_states(self) -> None:
        """Set the initial states for the shadow hand catch underarm task."""
        ...
        self.init_goal_pos = torch.tensor(
            [0.0, -0.79, 0.54], dtype=torch.float32, device=self.device
    ...
    def reset_init_pose_fn(self, init_states: list[EnvState], env_ids: torch.Tensor) -> list[EnvState]:
        ...
        if isinstance(init_states, list):
            ...
            for robot_id, robot in enumerate(self.robots):
                robot_dof_default_pos = self.robot_dof_default_pos[robot.name]
