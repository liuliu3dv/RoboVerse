...
class ShadowHandOpenOutwardCfg(BaseRLTaskCfg):
    ...
    def set_init_states(self) -> None:
        ...
        if self.obs_type == "state":
            ...
            self.img_h = 256
            self.img_w = 256
            self.cameras = [PinholeCameraCfg(name="camera_0", width=self.img_w, height=self.img_h, pos=(1.35, -0.2, 1.05), look_at=(0.0, -0.0, 0.5))] # TODO
    ...
    def observation_fn(self, envstates: list[EnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        door_right_handle_pos = door_right_handle_pos + math.quat_apply(door_right_handle_rot, self.y_unit_tensor * -0.5)
        door_right_handle_pos = door_right_handle_pos + math.quat_apply(door_right_handle_rot, self.x_unit_tensor * 0.39)
        door_right_handle_pos = door_right_handle_pos + math.quat_apply(door_right_handle_rot, self.z_unit_tensor * -0.04)
