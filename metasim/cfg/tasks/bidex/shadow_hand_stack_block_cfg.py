...
class ShadowHandStackBlockCfg(BaseRLTaskCfg):
    ...
    def set_init_states(self) -> None:
        ...
        if self.obs_type == "state":
            ...
            self.img_h = 256
            self.img_w = 256
            self.cameras = [PinholeCameraCfg(name="camera_0", width=self.img_w, height=self.img_h, pos=(-0.8, -0.35, 1.1), look_at=(0.0, 0.0, 0.5))] # TODO
    ...
    def observation_fn(self, envstates: list[EnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        if self.obs_type == "rgb":
            obs[:, 424:] = envstates.cameras["camera_0"].rgb.permute(0, 3, 1, 2).reshape(num_envs, -1) / 255.0 # (num_envs, H, W, 3) -> (num_envs, 3, H, W) -> (num_envs, 3 * H * W)
        return obs
...
def compute_hand_reward(
    ...
    stack_rew = torch.where(goal_dist2 < 0.07,
                    torch.where(goal_dist1 < 0.07,
                        (0.05-torch.abs(stack_pos1[:, 2] - left_object_pos[:, 2])) * 50 ,stack_rew),stack_rew)
