...
@configclass
class ShadowHandStackBlockCfg(BaseRLTaskCfg):
    """class for bidex shadow stack push block tasks."""
    ...
    def set_init_states(self) -> None:
        """Set the initial states for the shadow hand stack block task."""
        if self.obs_type == "state":
            ...
            self.obs_shape = 424 + 3 * self.img_h * self.img_w
        ...
        self.init_states = {
            "objects": {
                ...
                f"{self.current_object_type}_1": {
                    "pos": torch.tensor([0.0, 0.1, 0.625]),
    ...
    def observation_fn(self, envstates: list[EnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        Returns:
            ...
            424 - :     visual observation, currently RGB image (3 x 256 x 256)
        """
        if device is None:
            device = self.device
...
def compute_hand_reward(
    ...
    up_rew = torch.where(
        ...
        torch.where(left_hand_finger_dist < 0.5, (0.24 - goal_dist1 - goal_dist2) * 2, up_rew),
        up_rew,
    )
