...
class ShadowHandOpenInwardCfg(BaseRLTaskCfg):
    ...
    def set_objects(self) -> None:
        """Set the objects for the shadow hand open inward task."""
        self.objects.append(self.objects_cfg[self.current_object_type])

    def set_init_states(self) -> None:
        """Set the initial states for the shadow hand open inward task."""
...
def compute_hand_reward(
    ...
    up_rew = torch.where(
        ...
        torch.where(
            left_hand_finger_dist < 0.5, torch.abs(door_right_handle_pos[:, 1] - door_left_handle_pos[:, 1]) * 2, up_rew
        ),
        up_rew,
    )
