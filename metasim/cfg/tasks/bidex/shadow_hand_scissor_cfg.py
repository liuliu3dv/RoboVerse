...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (
        torch.norm(scissor_right_handle_pos - right_hand_ff_pos, p=2, dim=-1)
        + torch.norm(scissor_right_handle_pos - right_hand_mf_pos, p=2, dim=-1)
        + torch.norm(scissor_right_handle_pos - right_hand_rf_pos, p=2, dim=-1)
        + torch.norm(scissor_right_handle_pos - right_hand_lf_pos, p=2, dim=-1)
        + torch.norm(scissor_right_handle_pos - right_hand_th_pos, p=2, dim=-1)
    )
    left_hand_finger_dist = (
        torch.norm(scissor_left_handle_pos - left_hand_ff_pos, p=2, dim=-1)
        + torch.norm(scissor_left_handle_pos - left_hand_mf_pos, p=2, dim=-1)
        + torch.norm(scissor_left_handle_pos - left_hand_rf_pos, p=2, dim=-1)
        + torch.norm(scissor_left_handle_pos - left_hand_lf_pos, p=2, dim=-1)
    ...
    up_rew = torch.where(
        ...
        torch.where(left_hand_finger_dist < 0.7, (0.59 + object_dof[:, 0]) * 5, up_rew),
        up_rew,
    )
