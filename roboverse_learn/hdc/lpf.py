...
class ActionFilterButter(ActionFilter):
    ...
    def __init__(self, lowcut=None, highcut=None, sampling_rate=None, order=ACTION_FILTER_ORDER, num_joints=None):
        ...
        a_coeffs = []
        b_coeffs = []
        for i, (low, high) in enumerate(zip(self.lowcut, self.highcut)):
            if high <= 0.0:
                raise ValueError("Highcut must be > 0")

            b, a = self.butter_filter(low, high, sampling_rate, order)
            logging.info(
        ...
        super(self).__init__(a_coeffs, b_coeffs, order, num_joints, self.ftype)
...
class ActionFilterButterTorch(ActionFilterButter):
    ...
    def __init__(
        self, lowcut=None, highcut=None, sampling_rate=None, order=ACTION_FILTER_ORDER, num_joints=None, device="cpu"
    ):
        super(self).__init__(lowcut, highcut, sampling_rate, order, num_joints)
        self.device = device
    ...
    def filter_old(self, x):
        return super(self).filter(x)

    def reset_old(self, action_ids):
        return super(self).reset_by_ids(action_ids)
