...
class Humanoid_Batch:
    def __init__(
        self, mjcf_file="resources/robots/h1/h1.xml", extend_hand=True, extend_head=False, device=torch.device("cpu")
    ):
        ...
        if extend_hand:
            ...
            self._parents = torch.cat((mjcf_data["parent_indices"], torch.tensor([15, 19]))).to(
                device
            )  # Adding the hands joints
            arm_length = 0.3
            self._offsets = torch.cat(
                (mjcf_data["local_translation"], torch.tensor([[arm_length, 0, 0], [arm_length, 0, 0]])), dim=0
            )[None,].to(device)
            self._local_rotation = torch.cat(
                (mjcf_data["local_rotation"], torch.tensor([[1, 0, 0, 0], [1, 0, 0, 0]])), dim=0
            )[None,].to(device)
            self._remove_idx = 2
        else:
        ...
        if extend_head:
            ...
            head_length = 0.75
            self._offsets = torch.cat((self._offsets, torch.tensor([[[0, 0, head_length]]]).to(device)), dim=1).to(
                device
            )
            self._local_rotation = torch.cat(
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full=False, dt=1 / 30):
        ...
        if self.extend_hand and self.extend_head and pose.shape[-2] == 22:
            pose = torch.cat(
                [pose, torch.zeros(B, seq_len, 1, 3).to(device).type(dtype)], dim=-2
        ...
        for i in range(J):
            if self._parents[i] == -1:
                ...
                rot_mat = torch.matmul(
                    ...
                    torch.matmul(self._local_rotation_mat[:, (i) : (i + 1)], rotations[:, :, (i - 1) : i, :]),
                )
