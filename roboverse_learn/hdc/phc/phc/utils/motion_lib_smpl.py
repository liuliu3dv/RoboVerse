...
from phc.utils.motion_lib_base import FixHeightMode, MotionLibBase, compute_motion_dof_vels
...
class MotionLibSMPL(MotionLibBase):
    ...
    def load_motion_with_skeleton(
        ...
        for f in range(len(motion_data_list)):
            ...
            if masterfoot_config is not None:
                num_bodies = len(masterfoot_config["body_names"])
                pose_quat_holder = np.zeros([B, num_bodies, N])
                pose_quat_holder[..., -1] = 1
                pose_quat_holder[..., masterfoot_config["body_to_orig_without_toe"], :] = pose_quat_global[
                    ..., masterfoot_config["orig_to_orig_without_toe"], :
                ]
