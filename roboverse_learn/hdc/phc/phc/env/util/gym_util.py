...
def configure_gym(gym, config):
    ...
    if render:
        ...
        viewer = gym.create_viewer(
            sim, int(gymapi.DEFAULT_VIEWER_WIDTH / 1.25), int(gymapi.DEFAULT_VIEWER_HEIGHT / 1.25)
        )
...
def get_robot_states_from_torch_tensor(
    ...
    gttrny, gqny, vny, avny, info["root_yaw_inv"] = quaternion_math.compute_observation_return_info(
        global_quats, ts, vels, avels
    )
    joint_obs = np.concatenate(
        [gttrny.cpu().numpy(), gqny.cpu().numpy(), vny.cpu().numpy(), avny.cpu().numpy()], axis=-1
    )
