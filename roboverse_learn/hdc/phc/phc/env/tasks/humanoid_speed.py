...
class HumanoidSpeed(humanoid_amp_task.HumanoidAMPTask):
    ...
    def _build_marker_state_tensors(self):
        ...
        self._marker_actor_ids = self._humanoid_actor_ids + to_torch(
            self._marker_handles, device=self.device, dtype=torch.int32
        )
    ...
    def _reset_task(self, env_ids):
        ...
        change_steps = torch.randint(
            low=self._speed_change_steps_min,
            high=self._speed_change_steps_max,
            size=(n,),
            device=self.device,
            dtype=torch.int64,
        )
    ...
    def _compute_reward(self, actions):
        ...
        self.rew_buf[:] = self.reward_raw = compute_speed_reward(
            root_pos, self._prev_root_pos, root_rot, self._tar_speed, self.dt
        )
        ...
        if self.power_usage_reward:
            ...
            power_usage_reward = (
                -self.power_usage_coefficient * (power_usage_reward[:, 0] - power_usage_reward[:, 1]).abs()
            )
            power_usage_reward[self.progress_buf <= 3] = (
                0  # First 3 frame power reward should not be counted. since they could be dropped. on the ground to balance.
            )
    ...
    def _sample_ref_state(self, env_ids):
        (
            ...
            body_vel,
            body_ang_vel,
        ) = super()._sample_ref_state(env_ids)
