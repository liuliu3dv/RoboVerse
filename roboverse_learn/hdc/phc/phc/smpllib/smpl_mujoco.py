...
class SMPLConverter:
    ...
    def qpos_smpl_2_new(self, qpos):
        ...
        if len(qpos.shape) == 2:
            b_size = qpos.shape[0]
            jt_array = [
                qpos[:, smpl_qpos_addr[k][0] : smpl_qpos_addr[k][1]] if k in smpl_qpos_addr else np.zeros((b_size, 3))
                for k in new_qpos_addr.keys()
            ]
        else:
            jt_array = [
                qpos[smpl_qpos_addr[k][0] : smpl_qpos_addr[k][1]] if k in smpl_qpos_addr else np.zeros(3)
                for k in new_qpos_addr.keys()
            ]
    ...
    def qvel_smpl_2_new(self, qpvel):
        ...
        if len(qpvel.shape) == 2:
            ...
            jt_array = [
                qpvel[:, smpl_qvel_addr[k][0] : smpl_qvel_addr[k][1]] if k in smpl_qvel_addr else np.zeros((b_size, 3))
                for k in new_qvel_addr.keys()
            ]
        else:
            jt_array = [
                qpvel[smpl_qvel_addr[k][0] : smpl_qvel_addr[k][1]] if k in smpl_qvel_addr else np.zeros(3)
                for k in new_qvel_addr.keys()
            ]
        return np.hstack(jt_array)
    ...
    def get_new_jkp(self):
        return np.concatenate([
            [self.body_params[n][0]] * 3 if n in self.body_ws else [50] * 3 for n in self.new_joint_names[1:]
        ])

    def get_new_jkd(self):
        return np.concatenate([
            [self.body_params[n][1]] * 3 if n in self.body_ws else [5] * 3 for n in self.new_joint_names[1:]
        ])

    def get_new_a_scale(self):
        return np.concatenate([
            [self.body_params[n][2]] * 3 if n in self.body_ws else [1] * 3 for n in self.new_joint_names[1:]
        ])

    def get_new_torque_limit(self):
        return np.concatenate([
            [self.body_params[n][3]] * 3 if n in self.body_ws else [200] * 3 for n in self.new_joint_names[1:]
        ])
