        ...
        if rnn_format and self.is_rnn:
            for k, v in self.values_dict.items():
                if k not in self.special_names and v is not None:
                    self.values_dict[k] = (
                        self.values_dict[k].view(self.num_envs, self.horizon_length, -1).squeeze()
                    )  # Actions are already swapped to the correct format.
            if self.values_dict["rnn_states"] is not None:
                self.values_dict["rnn_states"] = [
                    s.reshape(self.num_envs, self.horizon_length, -1) for s in self.values_dict["rnn_states"]
    ...
    def _get_item_rnn(self, idx):
        ...
        if self.values_dict["rnn_states"] is not None:
            input_dict["rnn_states"] = [
                s[sample_idx, :].view(step_size * self.horizon_length, -1) for s in self.values_dict["rnn_states"]
            ]
