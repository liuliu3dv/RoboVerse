...
class IMAmpAgent(amp_agent.AMPAgent):
    ...
    def init_rnn(self):
        if self.is_rnn:
            ...
            self.states = [
                torch.zeros((s.size()[0], self.vec_env.env.task.num_envs, s.size()[2]), dtype=torch.float32).to(
                    self.device
                )
                for s in rnn_states
            ]
    ...
    def update_training_data(self, failed_keys):
        humanoid_env = self.vec_env.env.task
        joblib.dump(
            {"failed_keys": failed_keys, "termination_history": humanoid_env._motion_lib._termination_history},
    ...
    def _post_step_eval(self, info, done):
        ...
        termination_state = torch.logical_and(
            self.curr_stpes <= humanoid_env._motion_lib.get_motion_num_steps() - 1, info["terminate"]
        ...
        if self.curr_stpes >= curr_max or self.terminate_state.sum() == humanoid_env.num_envs:
            ...
            self.success_rate = (
                1 - np.concatenate(self.terminate_memory)[: humanoid_env._motion_lib._num_unique_motions].mean()
            )

            # MPJPE
            all_mpjpe = torch.stack(self.mpjpe)
            assert (
                all_mpjpe.shape[0] == curr_max or self.terminate_state.sum() == humanoid_env.num_envs
            ...
            all_body_pos_gt = [
                all_body_pos_gt[: (i - 1), idx] for idx, i in enumerate(humanoid_env._motion_lib.get_motion_num_steps())
            ]
