...
class UnrealEgoHeatmapSharedModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_HeatMap = torch.optim.Adam(
                params=self.net_HeatMap.parameters(), lr=opt.lr, weight_decay=opt.weight_decay
            )
    ...
    def evaluate(self, runnning_average_dict):
        ...
        for id in range(self.pred_heatmap_left.size()[0]):  # batch size
            ...
            loss_heatmap_left_id = self.lossfunc_MSE(self.pred_heatmap_left[id], self.gt_heatmap_left[id])
            loss_heatmap_right_id = self.lossfunc_MSE(self.pred_heatmap_right[id], self.gt_heatmap_right[id])
