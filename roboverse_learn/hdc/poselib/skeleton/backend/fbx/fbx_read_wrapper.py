...
def fbx_to_array(fbx_file_path, fbx_configs, root_joint, fps):
    ...
    command = f'{python_path} fbx_py27_backend.py "{fbx_file_path}" "{temp_file_path}" "{root_joint}" "{fps}"'
    logger.debug(f"executing command: {command}")
    os.system(command)
    logger.info("executing python script to read fbx data using Autodesk FBX SDK... done")
