...
class DemoCollector:
    def __init__(self, handler):
        ...
        if args.cust_name is not None:
            ...
            additional_str = ""
        self.base_save_dir = (
            f"humanoid_policy/roboverse_demo/demo_{args.sim}/{TaskName}-Level{args.random.level}{additional_str}/robot-{args.robot}-test"
        )
...
def main():
    ...
    camera = PinholeCameraCfg(data_types=["rgb", "depth"],pos=(1.5, -1.5, 1.5), look_at=(0.0, 0.0, 0.0), width=256, height=256)
    ...
    demo_indexer = DemoIndexer(
        save_root_dir=f"humanoid_policy/roboverse_demo/demo_{args.sim}/{TaskName}-Level{args.random.level}{additional_str}/robot-{args.robot}",
        start_idx=0,
        end_idx=max_demo,
        pbar=pbar,
    )
