...
def main():
    ...
    # task list
    # "/home/RoboVerse_Humanoid/roboverse_data/trajs/rlbench/"

    # all tasks
    # task_list = os.listdir("/home/RoboVerse_Humanoid/roboverse_data/trajs/rlbench/")
    # task_list.sort()
    ...
    for cur_task in task_list:
        ...
        scenario = ScenarioCfg(
            ...
            scene=args.scene,  # args.scene = None
            # objects=objects,
            cameras=[camera],
            random=args.random,
            try_add_table=args.table,  # args.table = True
            render=args.render,  # RenderCfg(mode='raytracing')
            split=args.split,  # all
            sim=args.sim,  # isaaclab
        ...
        # all_actions: N trajs (N is not same for different tasks)
        # Every traj has M frames (M is not same for different tasks),
        # Every frame is "dof_pos_target" of franka robotic arm

        # init_states[0]["robots"]["robot_franka_dst"] = {
        ...
        # 准备录像保存器
        # init states contain objects for different tasks
        obs, extras = env.reset(states=init_states)
        obs_saver = ObsSaver(video_path=f"./humanoid_retargeting/replay_franka_10_actions/{cur_task}/replay_{args.sim}.mp4")
        ...
        # N frames, inside is franka dict + dof_pos_target
        robot_joint = all_actions[0]  # only condier the first traj

        # [N, 26, 7], 26 is the link number of franka, not all links are movable
        robot_joint_list = []
        ...
        # forward kinematics to obtain the end effector position (wxyz_xyz)
        robot_pose = src_robot.forward_kinematics(robot_joint_array)  # [247, 26, 7]
        src_robot_links_names = src_robot.links.names

        # link mapping
        # map src franka panda_hand to dst franka panda_hand
        franka2franka = {
        'panda_hand': 'panda_hand',
        'panda_link0': 'panda_link0'
        }

        # target link to conduct inverse kinematics
        target_link_names = ["panda_hand"]

        # find the index
        # read positoin from src robot based on the index, and use it to do inverse kinematics for dst robot.
        inds = [src_robot_links_names.index(franka2franka[name]) for name in target_link_names]

        solutions = []
        for i in range(robot_pose.shape[0]):  # iterate on N frames

            solution = pks.solve_ik_with_multiple_targets(
                robot=tgt_robot,
                target_link_names=target_link_names,
                target_positions=np.array([robot_pose[i, inds[0], 4:]]),  # wxyz_xyz
