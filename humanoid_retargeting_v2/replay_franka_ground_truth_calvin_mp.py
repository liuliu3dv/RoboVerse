...
def main():
    ...
    robot_franka_dst = get_robot("franka_with_gripper_extension")
    ...
    # obs_saver = ObsSaver(video_path=f"./humanoid_retargeting/output/replay_franka_gt_calvin_actions/{cur_task}/replay_{args.sim}.mp4")
    ...
    for action in robot_joint:
        joint_angle = action['franka_with_gripper_extension']['dof_pos_target']
    ...
    for solution in solutions:
        ...
        # obs_saver.add(obs)
        body_names = obs.robots['franka_with_gripper_extension'].body_names
        body_state = obs.robots['franka_with_gripper_extension'].body_state  # [x,y,z,wxyz]
        root_state = obs.robots['franka_with_gripper_extension'].root_state
        robot_arm_pos_list.append(body_state)
        robot_arm_name = body_names

    # robot_arm_pos_list.append(root_state)
    data_list = [tensor.cpu().numpy() for tensor in robot_arm_pos_list]
    data_array = np.array(data_list)
    root_array = np.array(root_state.cpu().numpy())
    np.save('./humanoid_retargeting_v2/data/open_drawer_a_001.npy', data_array)
    np.save('./humanoid_retargeting_v2/data/open_drawer_a_001_root_state.npy', root_array)
